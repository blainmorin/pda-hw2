...
We then explored transformations of GFR, baseline creatine, and baseline urine protein. We used ladder plots, which creates scatterplots of all the combinations of common transformations:
...
In the ladder plots above, we are searching for the combinations that make a linear trend and create a good spread. From visual inspection it, it looks like the square root transformation of GFR and the log transformation of creatine and urine protein may be good transformation candidates. 

We checked these transformations on quantile-quantile plots:

```{r, echo = FALSE}
#.5 is good for GFR 
#log is best (but not great) for baseu and bascre

qqgfr = baseline %>% 
  ggplot(aes(sample = I(gfr^.5))) +
  stat_qq() +
  stat_qq_line() + 
  ggtitle("QQ Plot: GFR^.5") +
...
In the QQ plots above, the diagonal line represents a perfect normal distribution. We see that for both GFR and creatine, the sample quantile is less extreme than a normal distribution, which indicates that the sample data has thin tails. In the urine protein plot, we see that many of the points in the lower quantiles have the same value. This horizontal portion may represent a measurement threshold cutoff. 
...
lm2 = lm(I(gfr^.5) ~ log(bascre) + log(baseu) + sbase + AGE + as.factor(SEX) + as.factor(black))
lm3 = lm(I(gfr^.5) ~ log(bascre) + sbase + AGE + as.factor(SEX) + as.factor(black))
...
### black and basecre
black.cre = baseline %>%
  ggplot(aes(x = I(log(bascre)), y = I(gfr^.5))) + 
...
### black and baseu
black.eu = baseline %>%
  ggplot(aes(x = I(log(baseu)), y = I(gfr^.5))) + 
